...
resource "aws_eip" "nat" {
  ...
  tags = {
    Name        = "${var.project_name}-nat-eip-${count.index == 0 ? "A" : "B"}"
    environment = var.environment
  }
...
resource "aws_nat_gateway" "main" {
  ...
  tags = {
    Name        = "${var.project_name}-nat-gateway-${count.index == 0 ? "A" : "B"}"
    environment = var.environment
  }
...
resource "aws_route_table" "private" {
  ...
  tags = {
    Name        = "${var.project_name}-PrivateRouteTable-${count.index == 0 ? "A" : "B"}"
    environment = var.environment
  }
}
